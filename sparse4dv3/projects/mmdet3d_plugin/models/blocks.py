...
class DeformableFeatureAggregation(BaseModule):
    ...
    def _get_weights(self, instance_feature, anchor_embed, metas=None):
        ...
        if self.use_group_weights_kernel and not self.training and self.camera_encoder is not None:
            if self.projection_mat is None:
                self.projection_mat = metas["projection_mat"][:, :, :3].reshape(bs, self.num_cams, -1).contiguous()
            weights = group_weights_generation(instance_feature, anchor_embed, self.projection_mat, self.camera_encoder, self.weights_fc, self.num_groups)
