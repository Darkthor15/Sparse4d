...
class ImageCompressorNode(Node):
    def __init__(self):
        super().__init__('image_compressor_node1')
        self.bridge = CvBridge()
        self.sub = self.create_subscription(
            Image,
            '/CAMERA_FRONT_LEFT/image_8bit_color', 
            self.image_callback,
            10
        )
        self.pub = self.create_publisher(
            CompressedImage,
            '/CAMERA_FRONT_LEFT/image_rect_compressed',
            10
        )
    ...
    def image_callback(self, img_msg):
        ...
        compressed_msg.header.frame_id = 'base_link_fl'
